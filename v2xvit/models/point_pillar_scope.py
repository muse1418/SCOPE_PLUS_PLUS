...
class PointPillarScope(nn.Module):
    ...
    def forward(self, data_dict_list):
        ...
        for origin_data in data_dict_list:  
            ...
            # B, max_cav, 3(dt dv infra)
            prior_encoding = data_dict['prior_encoding']
            type_list.append(prior_encoding[:, :, -1])  # B,max_cav
        ...
        agent_type = type_list[0]  # B,max_cav
        agent_type_list = []  # B,max_cav
        for b in range(agent_type.shape[0]):
            temp = []
            for cav in range(agent_type.shape[1]):
                if agent_type[b, cav] == 1:
                    temp.append("infra")
                elif agent_type[b, cav] == 0:
                    temp.append("veh")
            agent_type_list.append(temp)
        ...
        if self.multi_scale:
            fused_feature, communication_rates, result_dict, commu_loss, gap_loss = self.fusion_net(spatial_features,
                                            psm_single,
                                            record_len,
                                            pairwise_t_matrix, 
                                            self.backbone,
                                            [self.shrink_conv, self.cls_head, self.reg_head], agent_type=agent_type_list)
            if self.shrink_flag:
                fused_feature = self.shrink_conv(fused_feature)
        else: 
            fused_feature, communication_rates, result_dict, commu_loss, gap_loss = self.fusion_net(spatial_features_2d,
                                            psm_single,
                                            record_len,
                                            pairwise_t_matrix)
