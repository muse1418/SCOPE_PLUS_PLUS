...
from v2xvit.models.comm_modules.attn_commu import AttnCommu,Exclusive_loss
...
class SCOPE(nn.Module):
    def __init__(self, args):
        ...
        self.attn_communication = AttnCommu(args['attn_communication'])
        self.exclusive_loss = Exclusive_loss()
    ...
    def forward(self, x, rm, record_len, pairwise_t_matrix, backbone=None, heads=None, agent_type=None):
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        ...
                        # 
                        communication_masks, communication_rates = self.attn_communication(x, record_len, deform_map)
                        
                        x = x * communication_masks
                        commu_loss = self.exclusive_loss(x, record_len)
                        # commu_loss = 0
                    else:
                ...
                for b in range(B):
                    ...
                    neighbor_feature = warp_affine_simple(node_features,
                                                    t_matrix[0, :, :, :],
                                                    (H, W))
                    H_origin, W_origin = deform_map[b].shape[2:]
                    confidence_maps = warp_affine_simple(deform_map[b], t_matrix[0, :, :, :], (H_origin, W_origin))
                    ...
                    if self.agg_mode == "Deform":
                        ...
                        if i == 0:
                            ...
                            deform_map_b = torch.cat([confidence_maps, padding_map],dim=0)
                        fused_feat, gap_loss = self.fuse_modules[i](neighbor_feature, deform_map_b,i,agent_type=agent_type[b])
                        x_fuse.append(fused_feat)
                        loss_list.append(gap_loss)
                    else:
                        x_fuse.append(self.fuse_modules[i](neighbor_feature))
                x_fuse = torch.stack(x_fuse)
                gap_loss = sum(loss_list)/len(loss_list)
                scale_loss_list.append(gap_loss)
        ...
        gap_loss = sum(scale_loss_list)
        return x_fuse, communication_rates, {}, commu_loss, gap_loss